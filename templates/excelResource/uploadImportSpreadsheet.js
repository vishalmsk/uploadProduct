...
const { 
  filter, hasIn, keys, map, uniq, isEmpty, replace,
  partialRight, pick, cloneDeep, compact, concat, 
  find, flatten, forEach, includes, indexOf, isEqual, 
  isNumber, lowerCase, max, remove, sum, trim, without,
  omitBy, multiply, divide,round,
...
var stopUpload = false; // make it true while debugging. You can view json but it wont be uploaded on server.
//===============================================================
export function uploadImportSpreadsheet(data) {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    if (currentProductId) {
      ...
      // _console("currentProductId nor found");
      resolve("Error: ProductId / selectedBook not found");
      return;
    }
    //---------------------------------------
    _console("automationObjects = ", automationObjects);
    ...
    //-----------------------------------------------
    if (stopUpload) {
      _console("stopUpload = ", stopUpload);
      return;
    }
    //================= Submit button code ==============
    ...
    if (automationDataToSave.length) {
      ...
      //=========================================
      //_console("dataWithoutIds = ", dataWithoutIds);
      ...
      if (dataWithoutIds.length) {
        ...
        createUserContentObjectPromise.then(async (data) => {
          _console("User content objects created", data);
          //userContentObjects = await getAutomationFiles({currentProductId,applicationType});
          //_console("112 userContentObjects = ",userContentObjects);
        });
        */
      }
      // _console("dataWithIds = ", dataWithIds);
      ...
      if (dataWithIds.length) {
        ...
        modifyUserContentObjectPromise.then(async (data) => {
          _console("User content objects modifed", data);
          //userContentObjects = await getAutomationFiles({currentProductId,applicationType});
         // _console("122 userContentObjects = ",userContentObjects);
        });
        */
      }
      // _console("123");
    ...
    // ================ Upload Resources data =========================
    //_console("Upload Successful");
    //=================== Submit button code ================
    //_console("automation data created");
    //_console("automationData = ", automationData);
...
async function updateResources(_value, _callback, applicationType) {
  ...
  return new Promise((resolve, reject) => {
    ...
    getHttpRequest(settings, function (_data) {
      ...
      // _console("sucess data _data = ", _data);
      _callback(_data);
      resolve(_data);
      //hideLoader();
    }, function (error) {
...
function modifyAssignmentDataInBulk(_data, _index, applicationType) {
  ...
  // _console("url = ",(app.serverPath + app.paths.modifyAssignmentBulk));
  return new Promise((resolve, reject) => {
    let settings = {
      "async": true,
      "crossDomain": true,
...
function createUserContentObject(data, applicationType) {
  ...
  //_console("api path = ",(app.serverPath + app.paths.createUserContentObject));
  return new Promise((resolve, reject) => {
    var settings = {
      async: true,
      crossDomain: true,
...
function modifyUserContentObject(data, applicationType) {
  ...
  //_console("api path = ",(app.serverPath + app.paths.modifyUserContentObject));
  return new Promise((resolve, reject) => {
    var settings = {
      async: true,
      crossDomain: true,
...
function getHttpRequest(settings, _callBackFn, _errorCallBackFn) {
  _console("getHttpRequest");
  _console("settings = ", settings);
...
async function createJSON(data) {
  return new Promise(async function (resolve, reject) {
     ...
     */
      ...
      // _console("resourcesForAutomation", resourcesForAutomation);
      // _console("tocForAutomation", tocForAutomation);
      // _console("passageForAutomation", passageForAutomation);
      //============ Sifter - 9884 - Product structure json tool ================== 
      //_console("894 currentProductId =", currentProductId);
      ...
      _console(sheetNamesAndPurpose.toc + " tab not present");
      if (!workbook.Sheets.hasOwnProperty(sheetNamesAndPurpose.resources)) {
        _console(sheetNamesAndPurpose.resources + " tab not present");
      }
      if (!workbook.Sheets.hasOwnProperty(sheetNamesAndPurpose.passage)) {
        _console(sheetNamesAndPurpose.passage + " tab not present");
      }
      if (!workbook.Sheets.hasOwnProperty(sheetNamesAndPurpose.productInfo)) {
        _console(sheetNamesAndPurpose.productInfo + " tab not present");
      }
      if (!workbook.Sheets.hasOwnProperty(sheetNamesAndPurpose.programCategories)) {
        _console(sheetNamesAndPurpose.programCategories + " tab not present");
      }
    }
    // _console("productStructureJson = ", productStructureJson);
    //==========================
...
function updateTOCdata(toc, resoursesUnitWiseData, resources) {
  ...
  toc.forEach((elm, index) => {
    if (elm["Resource Code"] === undefined) {
      ...
      //_console(index," :: resourceArr = ",resourceArr);
      if (resourceArr && resourceArr.length == 1) {
        //_console(index, elm["Unit (Parent)"], elm["Lesson (Child)"], "UPDATED");
        //_console("resourceArr = ",resourceArr);
...
function addPacingInformationInsideInteractiveResources(dataPassed, currentProductAssignments) {
  return addPacingInformation(dataPassed, currentProductAssignments);
}

function addPacingInformation(dataPassed = {}, currentProductAssignments) {
  let dataPassedKeys = keys(dataPassed);
  forEach(dataPassedKeys, (key) => {
    const keysData = dataPassed[key];
    if (isArray(keysData)) {
      if (key === RESOURCES_KEY) {
        if (keysData.length > 0) {
          dataPassed[key] = map(keysData, (resourceData) => {
            ...
            if (isResourceOfTypeInteractiveLessonOrAssessment(resourceDataToModify)) {
              const locationData = hasIn(resourceDataToModify, LOCATION) ? resourceDataToModify[LOCATION] : {};
              const associatedLessonId = hasIn(locationData, ASSOCIATED_ID) ? locationData[ASSOCIATED_ID] : "";
              let filteredAssignments = getAssignmentLinkedWithId(currentProductAssignments, associatedLessonId);
              if (filteredAssignments.length) {
                let pacingInfo = getPacingInformationIndividualResource(filteredAssignments);
                if (pacingInfo) {
                  resourceDataToModify = {
                    ...resourceDataToModify,
                    Pacing: pacingInfo
      ...
      if (key !== "name" && key !== "tiles" && key !== "passage" && key !== "EMPTY" && key !== LOCATION) {
        dataPassed[key] = addPacingInformation(keysData, currentProductAssignments);
      }
    }
  });
  return dataPassed;
...
function getAssignmentLinkedWithId(currentProductAssignments, associatedLessonId) {
  return filter(currentProductAssignments, (assignment) => {
    let flag = false;
    if (hasIn(assignment, "lessons") && hasIn(assignment.lessons, "0") && hasIn(assignment.lessons[0], "lessonId")) {
      flag = associatedLessonId !== "" && associatedLessonId === assignment.lessons[0].lessonId;
    }
    return flag;
  });
}

function getPacingInformationIndividualResource(lessonAssignments) {
  let clonedAssignments = cloneDeep(lessonAssignments);
  let infoToReturn = null;
  clonedAssignments = map(clonedAssignments, (assignment) => {
    return {
      ...assignment,
      presentation_data: getParsedPresentationData(assignment)
    }
  });

  let pacingInfo = compact(map(clonedAssignments, function (assignment) {
    const { presentation_data = {} } = assignment;
    const { pacing = "" } = presentation_data;
    return pacing;
  }));
  _console("getPacingInformationIndividualResource pacingInfo", pacingInfo)
  if (pacingInfo.length) {
    infoToReturn = getTotalPacingInformation(pacingInfo);
  }
  return infoToReturn;
}

function getTotalPacingInformation(pacingInfoArray) {
  let data = { [PACING_TIME_UNITS.DAY]: 0, [PACING_TIME_UNITS.HOUR]: 0, [PACING_TIME_UNITS.MINUTE]: 0 };
  forEach(pacingInfoArray, (pacingInfo) => {
    const spacingInfoSplitted = compact(split(pacingInfo, " "));
    if (spacingInfoSplitted.length === 2) {
      data[spacingInfoSplitted[1]] += +(spacingInfoSplitted[0]);
    }
  });
  if (data[PACING_TIME_UNITS.MINUTE] > 59) {
    var hourAddition = round(divide(data[PACING_TIME_UNITS.MINUTE], 60));
    data[PACING_TIME_UNITS.MINUTE] -= multiply(hourAddition, 60);
    data[PACING_TIME_UNITS.HOUR] += hourAddition;
  }

  if (data[PACING_TIME_UNITS.HOUR] > 23) {
    var dayAddition = round(divide(data[PACING_TIME_UNITS.HOUR], 24));
    data[PACING_TIME_UNITS.HOUR] -= multiply(dayAddition, 24);
    data[PACING_TIME_UNITS.DAY] += dayAddition;
  }

  data = omitBy(data, element => element === 0);

  if (hasIn(data, PACING_TIME_UNITS.DAY)) {
    if (keys(data).length > 1) {
      return `${data[PACING_TIME_UNITS.DAY]}-${data[PACING_TIME_UNITS.DAY] + 1} days`;
    } else {
      return `${data[PACING_TIME_UNITS.DAY]} day${data[PACING_TIME_UNITS.DAY] === 1 ? "" : "s"}`
    }
  } else if (hasIn(data, PACING_TIME_UNITS.HOUR)) {
    if (keys(data).length > 1) {
      return `${data[PACING_TIME_UNITS.HOUR]}-${data[PACING_TIME_UNITS.HOUR] + 1} hours`;
    } else {
      return `${data[PACING_TIME_UNITS.HOUR]} hour${data[PACING_TIME_UNITS.HOUR] === 1 ? "" : "s"}`
    }
  } else {
    return `${data[PACING_TIME_UNITS.MINUTE]} minute${data[PACING_TIME_UNITS.MINUTE] === 1 ? "" : "s"}`
  }
}

function getParsedPresentationData(element) {
  let data;
  if (element.hasOwnProperty('presentation_data') && typeof element.presentation_data === "string") {
    data = JSON.parse(element.presentation_data);
  }
  return (data != undefined) ? data : element.presentation_data;
}